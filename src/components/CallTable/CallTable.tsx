...
import { getCallTime, getCallDuration, getFullDate } from "../../lib/utils";
...
const CallTable: React.FC<CallTableProps> = ({}) => {
  ...
  const todayCalls = calls?.results
    .filter((call) =>
      searchValue && call.in_out === 1
        ? call.from_number.startsWith(searchValue)
        : call.to_number.startsWith(searchValue)
    )
    .filter((call) => call.date_notime === getFullDate());

  const yesterdayCalls = calls?.results
    .filter((call) =>
      searchValue && call.in_out === 1
        ? call.from_number.startsWith(searchValue)
        : call.to_number.startsWith(searchValue)
    )
    .filter((call) => call.date_notime === getFullDate(1));
      ...
      return (
        <tbody>
          {todayCalls &&
            todayCalls.map((call) => (
              <tr
                className="h-[65px] border-b border-[#EAF0FA] hover:bg-[#D4DFF3]/20 group/row"
                key={call.id}
              >
                <td className="table-header w-[40px] border-b border-[#fff] relative">
                  <input
                    type="checkbox"
                    className="hidden group-hover/row:inline-block checked:inline-block w-[17px] h-[17px] absolute top-[24px] left-[12px] text-[#002CFB] hover:text-[#0024CB] border-2 rounded-sm focus:ring-0 border-[#ADBFDF] cursor-pointer hover:border-[#002CFB] transition duration-150 ease-out"
                  />
                </td>
                ...
                <td className="table-header w-[129px] text-right text-[#122945] pr-[40px] relative">
                  <div
                    className={`${call.record && "group-hover:hidden"} pb-1`}
                  >
                    {call.time > 0 && getCallDuration(call.time)}
                  </div>
                  <div
                    className={`hidden ${
                      call.record && "group-hover/row:inline-block"
          ...
          {yesterdayCalls?.length! > 0 && (
            <tr className="h-[65px] border-b border-[#EAF0FA] relative">
              <td>
                <span className="absolute regular-text top-[28px] left-[41px] text-[#122945]">
                  вчера
                </span>
                <span className="text-[#899CB1] text-xs absolute top-[24px] left-[84px]">
